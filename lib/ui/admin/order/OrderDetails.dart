...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
        ...
        child: Container(
          ...
          child: ListView.builder(
            itemBuilder: (context, position) {
              return checkoutListItem(widget.cartList[position], position);
            },
  ...
  checkoutListItem(OrderItems cartList, int position) {
    var title;
    var qty;
    var url;
    if (widget.type == 0) {
      if (cartList.product?.brand?.title != null) {
        title = "${cartList.product.title} | ${cartList.product?.brand?.title}";
      } else {
        title = "${cartList.product.title}";
      }
      if (cartList.quantity != null) {
        qty = "  Qty:${cartList.quantity}";
      } else {
        qty = "";
      }
      url = widget.cartList[position].product?.imageUrl ?? "";
    } else {
